...
Two capabilities sit outside the x402 endpoint table below, because they are Firebase callables rather than paywalled routes: [bank accounts](/guides/agent-bank-accounts) and the [managed agent wallet](/guides/managed-agent-wallet). See [Beyond the x402 endpoints](#beyond-the-x402-endpoints).
...
| `createBankingRecipient`, `addBankingDestination`, `listBankingRecipients` | Register who gets paid and their bank account.                                                          |
| `createBankingTransfer`, `getBankingTransaction`, `listBankingTransactions` | Send a one-off payment and follow it to settlement.                                                    |
...
>
  Custody model, per-payment guardrails, and how failed settlements are
  reported.
</Card>

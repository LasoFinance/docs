...
An MPP client follows the same three steps: the 402 also carries a Machine Payments Protocol challenge in `WWW-Authenticate`, and the paid retry sends `Authorization: Payment ...` (USDC on Base) instead of a payment header. See [paying with MPP](/guides/how-x402-works#paying-with-mpp).
...
The `/auth` and `/get-card` endpoints return auth credentials (`id_token`, `refresh_token`) that can be used as Bearer tokens for authenticated endpoints like `/get-card-data`. Always save these tokens so you can use the `id_token` for authenticated requests and the `refresh_token` to get a new token when it expires. The curl examples reference it as the `$LASO_ID_TOKEN` environment variable.
...
| `/create-reloadable-card`   | POST   | Free (Bearer token)                                   | Create a reloadable card from the account holder's balance with the card issuer. Reusable by default.                                                                                                     |
| `/list-card-transactions`   | GET    | Free (Bearer token)                                   | List transactions on the account holder's reloadable cards.                                                                                                                                               |
| `/get-card-deposit-address` | GET    | Free (Bearer token)                                   | Get the holder's USDC deposit address at the card issuer, to fund a reloadable card balance (Base only).                                                                                                  |
| `/get-account-balance`      | GET    | Free (Bearer token)                                   | Get account balance                                                                                                                                                                                       |
| `/withdraw`                 | POST   | Free (Bearer token)                                   | Withdraw USDC to a Solana address you supply                                                                                                                                                              |
...
| `/get-payment-status`       | GET    | Free (Bearer token)                                   | Check the state of Venmo and PayPal payouts. Pass `payment_id` for one, `recipient_id` for one recipient's history, or neither for every payout sent.                                                     |
| `/fund-card-balance`        | GET    | Dynamic (\$5-\$1,000)                                 | Load a reloadable card balance. Pay from Base or Solana; Laso bridges to your deposit address at the card issuer over CCTP.                                                                               |
| `/withdraw-card-balance`    | POST   | Free (Bearer token)                                   | Withdraw unspent money off a reloadable card: the card issuer pays USDC on Base to an address you supply, within 1-3 business days.                                                                       |
...
| `/feedback`                 | POST   | Free (Bearer token)                                   | Send feedback about the API to the humans running Laso. Requires a completed real action; limited to 5/24h.                                                                                               |
...
Not everything an agent can do is a paywalled route. These are Firebase callables at `https://laso.finance/<name>`, authorized with the `id_token` from `/auth`, with arguments wrapped in a `data` object and replies in a `result` object.
...
| `getBankingTransaction`, `listBankingTransactions`                         | Follow a payment to settlement. Paying a destination is `GET /send-bank-payment`.                                                              |
...
| `getAgentSpendLimit`  | Read the per-payment spend limit the account owner set, plus its bounds. Read-only to an agent.                                                        |
...
- **Agentic Resource Discovery manifest**: [laso.finance/.well-known/ai-catalog.json](https://laso.finance/.well-known/ai-catalog.json)

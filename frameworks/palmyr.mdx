...
sidebarTitle: "Palmyr"
description: "Give your agent everything a real checkout requires — the Laso prepaid card, an email inbox for the order confirmation, and a phone number for SMS verification — paid per action in USDC via x402, with no signup or subscription."
---
...
[Palmyr](https://palmyr.ai) wraps [Laso prepaid cards](/guides/usa-prepaid-card) with the other things a real checkout asks for: an email inbox for the order confirmation and a phone number for SMS verification.
...
Cards follow [Laso's USA prepaid card rules](/guides/usa-prepaid-card) (US merchants, non-reloadable), and limits mirror Laso's: max 6 cards per agent per rolling 24 hours. Failed card purchases are refunded automatically.

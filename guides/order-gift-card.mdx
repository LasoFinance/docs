...
The catalog is large, so the workflow is "discover the filters, then filter." A single unfiltered request returns a `facets` object listing every valid value for `category`, `currency`, and `country`. (`$LASO_ID_TOKEN` is your session token, exported from your saved credentials.)

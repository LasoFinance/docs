...
<Steps>
  ...
  <Step title="Funds arrive">
    ...
    Confirm with `GET /get-payment-status` (free).
  </Step>
</Steps>
...
`/send-payment` answers as soon as the payout is dispatched (`state: "in-process"`), not when the money lands. `GET /get-payment-status` is the free follow-up: pass `payment_id` for one payout, `recipient_id` (from `GET /payment-recipients`) for one recipient's history, or nothing to list every payout the account has sent, newest first.
...
| State        | Meaning                                                       |
| ------------ | ------------------------------------------------------------- |
| `queued`     | Waiting for the account balance to cover it. Still in flight. |
| `in-process` | Handed to Venmo or PayPal. Normally completes within minutes. |
| `complete`   | The recipient has the money.                                  |
| `failed`     | Not delivered. The debited balance is credited back.          |
| `cancelled`  | Not delivered. The debited balance is credited back.          |

Poll every few minutes while a payout is `queued` or `in-process`, or [register a webhook](/guides/agent-webhooks) to be told when it completes. `state` is absent on payouts sent before state tracking existed.

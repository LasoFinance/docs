...
**Endpoint error.** The call completed and the service answered non-2xx. You get an ordinary result whose inner `status` is the service's code plus a normalized `error` sentence. Payment may or may not have settled (a settled one carries `txHash`); the record says `http_error` or `unsettled`. A wallet that cannot cover the quoted price is caught before signing and comes back as an inner 402 with `errorReason: "insufficient_funds"`, recorded as `insufficient_funds`. See [where payments are recorded](/guides/managed-wallet-spend-limit#activity-log).

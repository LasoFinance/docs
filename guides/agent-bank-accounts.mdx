...
- **Off-ramp**: the `fiatDestinationId` it pays out to. The off-ramp's internal funding address is deliberately not returned. To pay a destination, call `GET /send-bank-payment`, which opens the off-ramp and handles the funding leg for you.

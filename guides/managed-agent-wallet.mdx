...
**Two different ceilings, do not confuse them.** The \$10,000 figure is a blast-radius cap on *external* `url` payments only. The account's spend limit applies to *every* payment, external and Laso routes alike. On a Laso route such as `/send-bank-payment`, the spend limit is the **only** ceiling: if the human set it to \$50,000, a single \$12,000 bank payment is allowed and will go through. Do not refuse a route payment on the belief that \$10,000 caps it.

...
<Note>
  ...
  velocity limit yet, so repeated calls can still add up. Set `maxAmountUsdc` to
  the price you actually expect rather than leaving it at the ceiling.
</Note>

<Warning>
  **Pin the fee-inclusive total.** Most priced routes add a fee on top of the
  `amount` you request and round it up to the cent, so the challenge always
  costs more than the number you passed in. A `maxAmountUsdc` derived from the
  pre-fee amount lands a cent or two under the real price and refuses your own
  payment. Take the price from the 402 challenge's `amount` field (atomic units,
  divided by 1,000,000 for USDC) rather than recomputing it. For example
  `/send-bank-payment?amount=22841.89` adds a 0.25% fee of \$57.11 and quotes
  \$22,899.00, so `maxAmountUsdc` must be at least `22899`; `22898.99` is
  refused.
</Warning>

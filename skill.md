...
_Docs version `c96bd6bfd1c3`. This is a content hash of the Laso Finance agent docs. If it differs from the live version at https://laso.finance/.well-known/docs-version.json, your cached copy is stale. Refetch to get the current docs._
...
- **Off-ramp** — the `fiatDestinationId` it pays out to. Its internal funding address is deliberately not returned; pay it with `GET /send-bank-payment` instead.
...
- `amount` (required): On-card USD amount. Min $100, max $1,000. Use a whole dollar amount: the issuer only issues whole-dollar cards, so cents are rounded to the nearest dollar (`amount=250.50` yields a $251 card). The x402 payment is this amount plus a 3.8% fee.

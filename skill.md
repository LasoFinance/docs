...
{/* LASO-DOCS-VERSION:START (auto-generated by scripts/generate-docs-version.js, do not edit) _/}
*Docs version `4a4a659062b4`. This is a content hash of the Laso Finance agent docs. If it differs from the live version at https://laso.finance/.well-known/docs-version.json, your cached copy is stale. Refetch to get the current docs.*
{/_ LASO-DOCS-VERSION:END */}
...
- **Tools mirror the HTTP API one-to-one.** They are generated from the live `openapi.json`, so they always match the deployed interface — same names (snake_case of each `operationId`), same parameters, same responses. The managed-wallet calls (`get_agent_wallet`, `create_agent_wallet`, `agent_wallet_transfer`, `pay_x402_endpoint`) and a `search_docs` tool are included too, and the agent docs are exposed as MCP resources.
...
Then read the number and CVV with `GET /get-card-data?card_id=card_abc123`, which may require the account holder's approval (see above).

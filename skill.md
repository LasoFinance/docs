...
- `maxAmountUsdc`: hard cap on this single payment, in whole USDC (e.g. `0.008`). Must be positive and no greater than the built-in \$10,000 ceiling; it can only tighten that ceiling, never raise it. **That \$10,000 ceiling applies to external `url` payments only** — it bounds what an attacker-authored 402 challenge can pull from the wallet. It does not apply to Laso `route` payments, whose prices we author.
...
**Two different ceilings, do not confuse them.** The \$10,000 figure above is a blast-radius cap on *external* `url` payments only. The account's spend limit below applies to *every* payment, external and Laso routes alike. On a Laso route such as `/send-bank-payment`, the spend limit is the **only** ceiling: if your human set it to \$50,000, a single \$12,000 bank payment is allowed and will go through. Do not refuse a route payment on the belief that \$10,000 caps it.

...
_Docs version `aef3ed27a551`. This is a content hash of the Laso Finance agent docs. If it differs from the live version at https://laso.finance/.well-known/docs-version.json, your cached copy is stale. Refetch to get the current docs._
...
You need USDC to pay for the paywalled endpoints. Either let Laso custody a wallet for you (simplest, and the default) or bring a self-custody wallet from a provider.

**Option A — Laso managed account (default, no external wallet needed):**
...
# Attach their US bank account (returns destinationId)
curl https://us-central1-kyc-ts.cloudfunctions.net/addBankingDestination \
  -H "Authorization: Bearer YOUR_ID_TOKEN" -H "Content-Type: application/json" \
  -d '{"data":{"userId":"usr_...","recipientId":"...","destination":{"destination_type":"fiat_us","name":"Jane checking","aba_routing_number":"021000021","account_number":"123456789","account_type":"checking","account_holder_name":"Jane Doe","bank_name":"Chase"}}}'
...
Returns `depositCryptoAddress`: USDC sent to that Solana address pays out to the registered bank account automatically.

**You do not have to move the USDC yourself.** Once a destination exists, `GET /send-bank-payment?amount=250&destination_id=...` pays it over x402 in one call, opening the off-ramp account for you if there isn't one and handling the funding leg. Use `createBankingAccount` directly only when you want the deposit address to hand to something else. Listing your destinations is free at `GET /bank-recipients`.
...
- **Off-ramp** — `depositCryptoAddress` (send USDC there to trigger a payout) plus the `fiatDestinationId` it pays out to.
...
Account numbers come back masked to their last four digits; routing numbers are returned in full. Create new recipients and destinations with `createBankingRecipient` and `addBankingDestination`.

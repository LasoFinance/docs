...
_Docs version `ac57800ca7b8`. This is a content hash of the Laso Finance agent docs. If it differs from the live version at https://laso.finance/.well-known/docs-version.json, your cached copy is stale. Refetch to get the current docs._
{/* LASO-DOCS-VERSION:END */}

{/_ LASO-DOCS-VERSION:START (auto-generated by scripts/generate-docs-version.js, do not edit) _/}
_Docs version `873ca76eec36`. This is a content hash of the Laso Finance agent docs. If it differs from the live version at https://laso.finance/.well-known/docs-version.json, your cached copy is stale. Refetch to get the current docs._
{/_ LASO-DOCS-VERSION:END _/}
...
- **You are in the human's signed-in Laso browser:** use available WebMCP tools
  ...
  [WebMCP guide](https://docs.laso.finance/guides/webmcp) for supported actions.
...
**Windows PowerShell:** `curl` is an alias for `Invoke-WebRequest` and `~` is
not expanded for native programs, so use `curl.exe` and `$HOME`. The same two
steps are:

```powershell
New-Item -ItemType Directory -Force "$HOME\.laso" | Out-Null
curl.exe -fsS https://laso.finance/skills/laso-finance/scripts/setup.py -o "$HOME\.laso\setup.py"
python "$HOME\.laso\setup.py" --agent-name "Claude Code" --skill-dir "$HOME\.claude\skills\laso-finance"
```

Every later command in this file works the same way in PowerShell: replace
`python3 ~/.laso/setup.py` with `python "$HOME\.laso\setup.py"` (or `py` when
`python` is not on the PATH) and keep the flags unchanged.
...
- **Send an `Idempotency-Key` header on every POST you might retry.** Use a
  fresh UUID per intended action and reuse it on retries: the API returns the
  original response (marked `Idempotency-Replayed: true`) instead of
  performing the write twice. Applies to `/withdraw`, `/withdraw-card-balance`,
  `/create-reloadable-card`, `/cancel-intl-order`, `/refresh-card-data`,
  `/register-webhook`, `/delete-webhook`, and `/payment-recipients`.

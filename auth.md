...
_Docs version `c81a607e5d69`. This is a content hash of the Laso Finance agent docs. If it differs from the live version at https://laso.finance/.well-known/docs-version.json, your cached copy is stale. Refetch to get the current docs._
...
Keep the `lasoak_` key secret, like a password: it is the long-lived credential that mints these sessions, and it is the only credential you need to store. Everything else is derived from it.
...
**Then give `auth_url` to your human**, exactly as received and in full. It signs them into the web dashboard for this same account so they can watch balances, cards, and transfers. The `authToken` is a JWT hundreds of characters long, so copy the whole URL verbatim; a truncated link will not authenticate. It is single-use and time-limited, so call `agentAuth` again for a fresh one.
...
Surface `auth_url` to your human in full. Opening it signs them into the dashboard for your account, where they can inspect everything you have done. The link is one-time and short-lived; mint a fresh one when it expires. If you registered via `service_auth`, you already received an `auth_url` in Step 3 and do not need this call.

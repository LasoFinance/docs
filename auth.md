---
title: How an agent obtains Laso Finance credentials
...
# How an agent obtains Laso Finance credentials
...
*Docs version `d863f9e3da87`. This is a content hash of the Laso Finance agent docs. If it differs from the live version at https://laso.finance/.well-known/docs-version.json, your cached copy is stale. Refetch to get the current docs.*
...
**The MCP server is the exception, and it is standard OAuth.** `POST https://laso.finance/mcp` without a credential answers `401` with `WWW-Authenticate: Bearer resource_metadata="https://laso.finance/.well-known/oauth-protected-resource/mcp"`. An MCP client that follows it runs OAuth 2.1 with PKCE against the `authorization_endpoint`, `token_endpoint`, and `registration_endpoint` in the authorization-server document: the human signs in and approves in the browser, and the client receives a `lasoak_` API key as its access token. If you connect through an MCP client, let it do this; nothing below is needed.
...
```bash
curl https://laso.finance/agentAuth \
